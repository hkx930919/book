...
- **消息队列是一种数据结构，可以存放要执行的任务**。它符合队列“**先进先出**”的特点，也就是说<strong>要添加任务的话，添加
  到队列的尾部；要取出任务的话，从队列头部去取。</strong>
  - 正常的消息队列，即同步的队列
  - 异步任务列表：这个队列中维护了需要延迟执行的任务列表，包括了定时器和 Chromium 内部一些需要延迟执行的任务。
- 事件循环：渲染主线程会循环的从消息队列头部中读取任务，执行任务
  - 事件循环中处理完同步的消息队列后，判断延迟队列（即异步任务列表）中是否有任务执行，然后执行到期的异步任务。
- 渲染进程有个 IO 线程用来接收其他进程传进来的消息，接受消息后会将这些消息组装成任务发送给渲染主进程，推入到消息队列中。
- 渲染进程中的主线程会设置一个退出标志的变量，在每次执行完一个任务时，判断是否有设置退出标志，如果退出则中断当前的所有任
  务，退出线程。

### 2 定时器

- 当设置的定时时间到了，但是当前任务执行过久，会影响到定时器的执行时间。所以设置的 timeout 不一定准确
- 定时器会加入到异步任务队列，未激活的页面定时器执行的最小间隔是 1000ms，目的是为了优化后台页面的加载损耗以及降低耗电量
- setTimeout 存在嵌套调用，那么系统会设置最短时间间隔为 4 毫秒
- 定时器的最大值是 28.4 天左右，32bit 存放的数字 2147483647 毫秒

### 3 XMLHttpRequest

- 同步回调和异步回调
  - 同步回调：即函数在主函数内部执行
  - 异步回调：回调函数在主函数外部执行的过程称为异步回调。一般有两种方式：一是把异步函数添加到微任务队列中，二是把异步函
    数封装成一个任务，放到异步任务队列中。
- 跨域问题
- https 和 https 混合问题，大部分资源可以加载，但是 http 请求会被浏览器拦截掉

### 4 宏任务和微任务

- 宏任务：消息队列中的任务都是宏任务，包含 js 脚本执行事件（即运行一段代码）、渲染事件（如解析 DOM、计算布局、绘制）、用
  户交互事件（如鼠标点击、滚动页面、放大缩小等）、网络请求完成、文件读写完成事件
- 微任务：就是一个需要异步执行的函数，执行时机是在**主函数执行结束之后、当前宏任务结束之前**。
  - 目前只有 promise 和 MutationObserver 可以创建微任务队列
- 每个宏任务都有个微任务队列，当前宏任务结束之前会检查微任务队列，并执行微任务队列。把执行微任务的时间点称为检查点
  - 执行微任务队列会延长当前宏任务执行的时间
- 事件循环时先执行消息队列中的宏任务，执行过程中产生的微任务放入微任务队列，然后再当前宏任务结束之前执行微任务队列，执行
  完毕后进入下一个事件循环，再从消息队列拿出一个宏任务开始执行。

### promise

promise 通过回调函数延迟绑定、回调函数返回值穿透和错误冒泡解决回调地狱问题。promise 的实现主要从这三个特点实现

### async/await

`async/await` 在不阻塞主线程的情况下使用同步代码实现异步访问资源的能力，并且使得代码逻辑更加清晰。

- 协程：一种比线程更加轻量级的存在，可以把协程看成是跑在线程上的任务，一个线程上可以存在多个协程，但是在线程上同时只能执
  行一个协程
- 生成器（generate 函数）：一个带星号函数，而且是可以暂停执行和恢复执行的。生成器就是协程的一种实现方式
- async:一个通过异步执行并隐式返回 Promise 作为结果的函数
- await:执行到 await 语句时，会默认创建一个 Promise 对象，加入微任务队列中。然后 JavaScript 引擎会暂停当前协程的执行，将
  主线程的控制权转交给父协程执行，同时会将 promise 对象返回给父协程。父协程监听 promise 的变化，且继续执行父协程的内容。
  执行完毕后，检查微任务队列，然后将协程转移出去，之后执行 await 后的内容
